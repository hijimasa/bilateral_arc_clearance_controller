...
- 全方向policyは、回避を横速度で行いヨーレートを姿勢規範に使う。候補格子は「前進速度 × 横速度」で、
  次元は差動二輪の「前進速度 × ヨーレート」と同じ2次元である。候補数は同数ではない。
  ヨーレートは候補生成の前に確定するため、採点し接触判定する軌道と実際に走る軌道が一致する。
  狭所では左右クリアランスの不均衡を姿勢規範へ加え、
...
同梱の全方向設定の値（`limits.v_max` 0.4、`limits.vy_max` 0.3、`limits.w_max` 1.0、`v_samples` 5、`vy_samples` 15、
...
この節の記述が過去に誤っていた点（R19 H3: 現在速度によっては156や182になると書かれていた）は、
[R19対応記録](reviews/r19-2026-09-02-response.md)に旧記述の引用とともに記録がある。

#### 全方向モデルの左右均衡項と `avoid_margin.side`（実測）

同梱の全方向設定の車体幅0.5 mで`avoid_margin.side`を0.5にするとcapは0.75 mとなり、通常の1.6 m通路では遠い側が「開いている」と
判定されて左右均衡項は一度も発火しない。
...
**帯**であり、格子がその帯の内側に落ちるか外側に落ちるかはほぼ偶然である。
実測: 0.702〜0.708が0.1863〜0.1952（0.005格子は0.705しか踏まない）、0.721〜0.729が
...
刻み幅での測定であり、それより狭い帯の不在は示せていない）。
同梱の0.9はこの帯から十分離れており、0.8（0.016102）も0.9（0.015945）とほぼ同値である。つまり同梱の0.9はこの境界に対する余裕であって、
必須の値ではない。
...
旧版のこの節を見て値を選んだ読者に向けた訂正である。
R19 H4: ここには「0.705（0.1900）・0.710（0.1869）で
戻り、0.715以降で安定」と書かれていたが、0.1900 / 0.1869は実際には0.695 / 0.705の値で1〜2格子ずれて
おり、0.725という反例が抜けていた。R19の対応1巡目はこれを「0.017未満で安定するのは0.730以降」と
直したが、それは0.005格子上の言明としてしか正しくない（0.737〜0.738が反例）。

執筆過程の訂正（R19 4巡目——807点の区間を「0.0157〜0.0163」と書いていた、R18 M6——0.0119 / 0.139が
旧来の厚みゼロrigの値だった）は、[R19対応記録](reviews/r19-2026-09-02-response.md)と
[R18対応記録](reviews/r18-2026-09-02-response.md)に旧記述の引用とともに記録がある。
...
以上が測った4基準である。R19 M3の対応1巡目が書いた迂回72・非完走16は、この窓つきの基準——
