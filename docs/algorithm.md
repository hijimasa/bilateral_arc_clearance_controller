...
- 全方向policyは、回避を横速度で行いヨーレートを姿勢規範に使う。候補格子は「前進速度 × 横速度」で、
  ...
  `w_samples` 25）で現在速度 (0.20 m/s, 0.0 rad/s) から測るとomni 96・差動二輪130であり、同じ設定でも
  現在速度が変われば差動二輪側は156や182になる（R18 M5）。ヨーレートは候補生成の前に確定するため、
  ...
  **測定条件を明記する**（R15 M12は、条件を書かない数値表がレビューアの手元で再現しないことを指摘した）。
  ...
  1.6 m通路（壁厚0.10 m）での平均横偏差は0.9で0.0159 m、0.5で0.1412 mになる。遷移は非単調で、0.05 mを
  最初に下回るのは0.700だが0.705・0.710は再び0.18超に戻り、0.017未満で安定するのは0.715以降である。
  0.8でも0.9とほぼ同じ値に達する。R18 M6: ここに書かれていた0.0119 / 0.139という対は、この段落が宣言する
...
- **薄い壁に沿って、ほぼ平行に進入する場合の接触回避。** BACは与えられた点群しか見ない。厚みの無い
  ...
  0.488 mと0.498 mの2セルで車体接触が残る（R18 H5。以前ここには「その位置へ入らなくなった」と書いて
  いたが偽である）。厚み0.10 mの同じ通路は全101セルが接触なしで完走する。実際の壁は面を持つため、
  厚みゼロは評価上の縮退であり、回帰試験は厚みのある壁で行う。分解能を上げる（2880本以上）か、壁に
  厚みを与えるといずれも解消する。
  なお4条件（幅1.1・1.2 m × 厚み0.10 m・厚みゼロ）計404セルの不合格91件の内訳は、接触2件、通路外への
  迂回73件、非完走16件である。接触は厚みゼロの1.2 m通路にのみ現れる
