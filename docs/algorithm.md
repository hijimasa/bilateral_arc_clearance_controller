...
- 全方向policyは、回避を横速度で行いヨーレートを姿勢規範に使う。候補格子は「前進速度 × 横速度」で、
  ...
  進入オフセット0.30 mでの実測平均横偏差は、通路幅1.1 / 1.2 / 1.3 / 1.6 / 1.8 mに対し
  0.013 / 0.014 / 0.012 / 0.012 / 0.016 mであり、同条件の差動二輪（0.025 / 0.026 / 0.027 / 0.037 /
  0.035 m）より2〜3倍小さい。幅1.0 mでは入口で停止する。孤立障害物の迂回では`max|w|`が0.036 rad/s
  （差動二輪は0.313 rad/s）で、ヨーをほとんど使わずに横へ避ける。決定論的試験17件（単体9・閉ループ8）を
  通過する。
