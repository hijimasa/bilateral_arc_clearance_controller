...
- 全方向policyは、回避を横速度で行いヨーレートを姿勢規範に使う。候補格子は「前進速度 × 横速度」で、
  差動二輪の「前進速度 × ヨーレート」と同じ大きさである。ヨーレートは候補生成の前に確定するため、
  ...
  **測定条件を明記する**（R15 M12は、条件を書かない数値表がレビューアの手元で再現しないことを指摘した）。
  ...
  この条件での平均横偏差は、通路幅1.1 / 1.2 / 1.3 / 1.6 / 1.8 mに対し
  0.0124 / 0.0143 / 0.0123 / 0.0119 / 0.0155 mであり、同条件の差動二輪（0.0253 / 0.0261 / 0.0272 /
  0.0367 / 0.0348 m）に対して1.8〜3.1倍小さい。`avoid_margin.side`を下げると左右均衡項が発火せず、
  1.6 m通路での平均横偏差は0.9で0.0119 m、0.5で0.139 mになる。発火境界は0.70と0.75の間にあり、
  0.8でも0.9とほぼ同じ値に達する。
  **進入オフセットには限界がある**。1.2 m通路では0.30〜0.40 mで通過するが0.45 mでは入口で停止し、
  1.1 m通路では0.36 m以上で通路の外側を迂回する。いずれも接触ではない。
  孤立障害物の迂回では`max|w|`が0.036 rad/s（差動二輪は0.313 rad/s）で、ヨーをほとんど使わずに横へ
  避ける。決定論的試験22件（単体10・閉ループ12）を通過する。
