...
|---|---:|---:|---:|---|
| BAC | 54/54 (100%) | 0 | 27.7 s | 全条件で完走。狭所・劣化条件でも減速幅が小さい |
| DWB | 50/54 (93%) | 1 | 25.1 s | 通常条件は高速。出現障害物で失敗、0.25 m ズレは 0.01 m の擦過通過 |
| MPPI | 51/54 (94%) | 0 | 29.4 s | 通常条件と動的横断に強い。出現障害物で失敗 |
...
| 0.20 | 28.8 s / **0.23** | 25.8 s / 0.05 | 28.8 s / 0.06 | 27.8 s / 0.06 |
| 0.25 | 28.8 s / **0.23** | 27.2 s / **0.01** | 36.1 s / 0.06 | 0/2（中断） |

BAC の到達時間とクリアランスは**ズレ量に対して不変**である（全域で 28.8 s / 0.23）。追従系は
ズレにほぼ 1:1 でクリアランスを失い、run により衝突・中断・数 cm の擦過通過（DWB の 0.25 m は
最小 0.01 m）のいずれかに至る。RPP は 0.25 m で完走できず、MPPI は完走するが
クリアランス 0.06 m・時間 +25% と大きく劣化する。
...
| DWB | 1 | 7 | 11 | 0.000 | 0.235 |
| MPPI | 0 | 1 | 4 | 0.027 | 0.277 |
| RPP | 0 | 2 | 5 | 0.029 | 0.261 |
