...
`results/summary.csv` は 16 シナリオを含む（BAC は全シナリオ各 3 run の 48 episode、
他 controller は各 1〜5 run 合計 40 episode）。

| controller | 成功 episode | 成功時平均到達時間 | 観察された傾向 |
|---|---:|---:|---|
| BAC | 48/48 (100%) | 29.3 s | 全条件で完走。狭所・劣化条件でも減速幅が小さい |
...
- 通常の開空間・広路・狭路では全 controller が成功し、BAC は概ね 1.1〜1.3 倍遅かった
  （1.5 m 限界通路 29.6 s vs RPP 24.8 s、開空間 16.4 s vs 14.9 s）。速度規制を衝突コース
