...
`results/summary.csv` は Z 字路 2 種を含む 18 シナリオ（BAC は各 5 run の 90 episode、
他 controller は各 1〜5 run 合計 46 episode）。

| controller | 成功 episode | 成功時平均到達時間 | 観察された傾向 |
|---|---:|---:|---|
| BAC | 90/90 (100%) | 29.4 s | 全条件で完走。狭所・劣化条件でも減速幅が小さい |
| DWB | 40/46 (87%) | 24.6 s | 通常条件は高速。強い経路横ずれと出現障害物で失敗 |
| MPPI | 42/46 (91%) | 28.4 s | 通常条件と動的横断に強い。出現障害物と強い横ずれの一部で失敗 |
| RPP | 40/46 (87%) | 24.1 s | 最速。経路上の出現障害物と強い横ずれで失敗 |
...
- 通常の開空間・広路・狭路では全 controller が成功し、BAC は概ね 1.1〜1.3 倍遅かった
  （1.5 m 限界通路 28.8 s vs RPP 24.8 s、開空間 16.0 s vs 14.9 s）。速度規制を衝突コース
  限定にした改修（v0.1.0）以降、狭所でも巡航速度を大きく落とさない。
- 逆向き 2 連コーナーの Z 字路（1.7 m 幅）は全 controller が成功し、追従系が速い
  （RPP 20.8 s、DWB 21.2 s、BAC 36.5 s）。BAC の慎重な二重旋回は時間を要するが、
  自己位置ズレ変種（`corridor_zigzag_locdrift`）でもクリアランスを保って完走する。
...
結論は、以下の 3 点は原理的にプランナでは提供できず、実行層(controller)にのみ実装可能である。

1. **情報の非対称性**: プランナは地図とポーズ推定の上で計画するため、ポーズ推定自体が誤って
   いるとき真の相対幾何を知る手段がない。センサ直結の実行層は座標系誤差に構造的に免疫を持つ。
...
積んで `corridor_locdrift_15x`(1.5 m 通路 + 0.15 m 自己位置ズレ)を再走した(各 2 episode)。

| 上流対策 | DWB | MPPI | RPP | BAC(対策なし) |
|---|---|---|---|---|
| なし(基準) | 衝突 | 2/3 成功 | 中断 | **成功 5/5**(29.6 s, clr 0.23) |
...
全シナリオ・全 episode(BAC 90、他 各 46)の障害物への最接近距離:

| controller | 衝突 | 接近 <0.05 m | 接近 <0.10 m | 最悪値 [m] | 中央値 [m] |
|---|---:|---:|---:|---:|---:|
| BAC | 0 | 0 | 0 | 0.102 | 0.282 |
| DWB | 2 | 8 | 13 | 0.000 | 0.205 |
| MPPI | 0 | 2 | 3 | 0.026 | 0.275 |
| RPP | 0 | 3 | 6 | 0.021 | 0.261 |

幅を掃引した狭窄部(車体 0.95 m)では、追従系の速度は狭窄幅に応答しない(DWB/RPP は片側
余裕 5 cm の 1.15 m 隙間でも 0.40 m/s)。BAC のみ 0.39 → 0.28(幅 1.35)→ 0.10(1.25)→
進入拒否(1.15、衝突なしで停止)と段階応答し、速度が近接度の関数になっている。

以上から、本パッケージの位置づけは「最速の controller」ではなく、**上流(自己位置・地図・
プランナ・再計画)の状態に依らず成立する、実行層の測定可能な頑健性契約**である:
(a) 全 90 episode で障害物 0.10 m 以内に非侵入・衝突ゼロ、(b) 速度は障害物への近接に単調応答、
