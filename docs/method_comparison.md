...
|---|---:|---:|---:|---:|---|
| BAC | 54/54 (100%) | 0 | 27.6 s | 28.4 s | 全条件で完走。狭所・劣化条件でも減速幅が小さい |
| DWB | 49/54 (91%) | 1 | 27.9 s | 25.2 s | 通常条件は高速。出現障害物で全滅、0.25 m ズレで衝突 |
| MPPI | 51/54 (94%) | 0 | 29.0 s | 27.2 s | 通常条件と動的横断に強い。出現障害物で失敗 |
| RPP | 48/54 (89%) | 0 | 24.2 s | 24.8 s | 最速。出現障害物と 0.25 m 自己位置ズレで中断 |

平均到達時間は成功 episode のみの算術平均で、失敗を罰する総合スコアではない。controller ごとに
成功したシナリオ集合も異なるため、到達時間の横比較も参考値である。DWB の平均が中央値より
大きいのは `corridor_extreme_offset` で 168 秒に及ぶ復帰動作が 1 件あるためで、通常条件の
...
- `corridor_extreme_offset`: DWB 2/3(1 件は 168 秒の復帰動作の末に成功、1 件は中断)。他は 3/3。
- 通常の開空間・広路・狭路では全 controller が成功し、BAC は概ね 1.05〜1.1 倍遅かった
  (開空間 16.0 s / RPP 14.8 s、広路 26.0 / 24.8、狭路オフセット 30.9 / 28.8)。
  速度規制を衝突コース限定にした改修以降、狭所でも巡航速度を大きく落とさない。
- 逆向き 2 連コーナーの Z 字路（1.7 m 幅）は全 controller が成功し、追従系が速い
  （RPP 20.9 s、DWB 21.2 s、MPPI 27.2 s、BAC 31.5 s）。旋回時の矩形コーナー掃引を正確化した結果、
  BAC の二重旋回は保守的である。自己位置ズレ変種（`corridor_zigzag_locdrift`）でも
  クリアランスを保って完走する。BAC の全 54 episode 中の最接近 0.139 m もこのシナリオで出ている。
...
このsweepでは、BACの到達時間に表示桁での変化はなく、クリアランスは0.22〜0.23 mだった。
比較対象ではズレの増加に伴うクリアランス低下が観測され、0.25 mではDWBが1/2、RPPが0/2となった。
MPPIは2/2完走したが、クリアランス0.08 m、到達時間51.8 sだった。この結果は試した設定と
...
ギャップスイープ(24 episode)は domain プール 90 未満で再利用自体が発生しないため、旧 runner
生成のまま用いている。
...
(a) 54 episodeで衝突0・最接近0.139 m、(b) 衝突コース上の障害物に対する段階減速、
(c) 1.15 m開口での前進拒否、(d) 観測範囲内での後退escapeを確認した。これらはセンサ、TF、odom、
