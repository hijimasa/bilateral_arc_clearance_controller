...
BAC は、矩形の差動二輪またはAckermannロボットがローカル経路を追従しつつ、2D LiDAR または
...
BAC は各tickで、差動二輪またはAckermannの候補policyから1本の定曲率円弧を選ぶため、計算と
チューニング対象が小さく、左右クリアランスの意味をログから追いやすい。その代わり、S字の制御列、
動的障害物の将来予測、holonomicな横移動は表現しない。
...
| BAC | 定曲率円弧 `(v,w)` | 左右円弧クリアランス | 左右差を直接最小化 | 狭路の幾何中心、差動二輪・Ackermann policy | 静的点群・holonomic未対応、操舵／角速度過渡は未評価 |
