...
plan の TF 変換修正（リリースレビュー Critical 1）後の単一リビジョンで全 controller を
...
|---|---:|---:|---:|---|
| BAC | 54/54 (100%) | 0 | 28.7 s | 全条件で完走。狭所・劣化条件でも減速幅が小さい |
| DWB | 50/54 (93%) | 2 | 24.7 s | 通常条件は高速。出現障害物と 0.25 m 自己位置ズレで衝突・失敗 |
| MPPI | 51/54 (94%) | 0 | 28.4 s | 通常条件と動的横断に強い。出現障害物で失敗 |
...
- `appearing_obstacle`: BAC 3/3、DWB/MPPI/RPP は各 0/3（DWB は衝突 1 を含む）。経路上へ
  後から現れた障害物を BAC が局所的に迂回した。
- `corridor_locdrift_15x`（1.5 m 通路 + **0.25 m** 自己位置ズレ）: BAC 3/3、DWB 0/3（衝突）、
  MPPI 3/3（ただしクリアランス 0.08 まで低下）、RPP 0/3（中断）。ズレ量スイープは次節。
...
| 0.20 | 28.8 s / **0.23** | 30.2 s / 0.05 | 28.6 s / 0.07 | 1/2 失敗 |
| 0.25 | 28.8 s / **0.22** | **0/2 衝突** | 46.2 s / 0.08 | 0/2 失敗 |

BAC の到達時間とクリアランスは**ズレ量に対して不変**である（28.8 s / 0.22〜0.23）。追従系は
ズレにほぼ 1:1 でクリアランスを失い、0.20〜0.25 m で衝突または中断に至る。MPPI は 0.25 m でも
完走するが、クリアランス 0.08 m・時間 +60% と大きく劣化する。
...
1. **情報の非対称性**: プランナは地図とポーズ推定の上で計画するため、ポーズ推定自体が誤って
   いるとき真の相対幾何を知る手段がない。ロボット座標系のセンサ観測を制御周期で使う実行層は、
   地図↔オドメトリ間の座標系誤差の影響を受けない(センサ自体の外部パラメータ誤差や遅延は別途残る)。
...
|---|---|---|---|---|
| なし(基準) | 衝突 | 成功(clr 0.08) | 中断 | **成功**(28.8 s, clr 0.22) |
...
|---|---:|---:|---:|---:|---:|
| BAC | 0 | 0 | 0 | 0.109 | 0.233 |
| DWB | 2 | 7 | 11 | 0.000 | 0.212 |
| MPPI | 0 | 1 | 5 | 0.032 | 0.286 |
| RPP | 0 | 3 | 5 | 0.029 | 0.263 |
...
(a) 全 54 episode(単一リビジョン・全 18 シナリオ)で障害物 0.10 m 以内に非侵入・衝突ゼロ、(b) 速度は障害物への近接に単調応答、
