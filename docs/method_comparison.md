...
BAC は各tickで、差動二輪・Ackermann・全方向の候補policyから1本の定曲率の車体運動を選ぶため、計算と
チューニング対象が小さく、左右クリアランスの意味をログから追いやすい。その代わり、S字の制御列と
動的障害物の将来予測は表現せず、公平条件benchmarkの対象は差動二輪モデルのみである。
...
| BAC | 定曲率の車体運動 `(v, vy, w)` | 左右円弧クリアランス | 左右差を直接最小化 | 狭路の幾何中心、差動二輪・Ackermann・全方向policy | 静的点群のみ、操舵／角速度過渡は未評価。公平条件benchmarkは差動二輪のみ |
