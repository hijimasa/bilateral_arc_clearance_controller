...
domain free-list とプロセス回収の連動により構造的に保証される。本ページの数値を生成した時点の
runner は通番の剰余で domain を割り当てていたため、事後監査
(`nav2_benchmark/scripts/audit_legacy_domains.py`)を行った: 正準 216 episode で domain を共有した
162 対すべてについて、ジョブプールの同時実行上限から**同時に生存し得なかった**ことが完了時刻から
確定する(並列数を保守的に 8 と仮定しても同じ)。ズレ量スイープ(32)とギャップスイープ(24)は
そもそも domain 再利用が発生しない規模である。
