...
| `velocity_min` | 0.005 | Output linear speeds below this value are rounded to zero [m/s]. Ackermann zeroes the whole command, since a yaw rate without speed is not realizable. For when it applies, and the R19 L10 correction, see [When the deadband (`velocity_min` / `angvel_min`) applies](#when-the-deadband-velocity_min--angvel_min-applies) |
| `angvel_min` | 0.01 | Differential drive AND the holonomic model: output angular speeds below this value are rounded to zero [rad/s]. For when it applies, the four routes and the measured split, see [When the deadband (`velocity_min` / `angvel_min`) applies](#when-the-deadband-velocity_min--angvel_min-applies) |
