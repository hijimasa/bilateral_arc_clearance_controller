...
- R16（`a9c6c31` を対象、最重点は未レビューの `433068a`）は判定 **Hold**、対応未了。R15 H4は格子点上
  でのみ解消しており、0.002 m刻みの掃引で接触が再現する。High 5件は統合を阻む
...
| R16 | R15対応の再レビュー。R15 H4が格子点上でのみ解消（0.002 m刻みで接触再現、侵入0.23 m）、deadbandが判定後に走りpublishされるtwistで不変条件が破れる、速度governorが前進軸のまま、R15対応が新設したコードが試験から未実行 | 未対応（判定 **Hold**） | [指摘](reviews/r16-2026-09-02-findings.md) |
