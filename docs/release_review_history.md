...
- R18（`ba544e2` を対象、最重点は未レビューの `364fe1a` / `ba544e2`）は判定 **Hold**、対応未了。
  ...
  書き換えたものである。R18の記録commitは本commitである

全18回のリリースレビューのうち、R14までのCritical / High / Mediumは対応済みである。R15〜R18は
...
| R18 | R17対応の再レビュー（最終）。掃引の安全不変条件が接触距離を被検コード自身から取る自己参照で、車体横幅を半分と見なす変異が「違反0」と印字して通過する。R17対応資料がinstall対象2ファイルの修正を実施せずに報告、deadband分岐のコメントの実測値（14件中14件）が再現しない（7/14）、「判別する回帰は用意できない」が偽、薄壁記述と英語版履歴が偽 | 対応未了 | [指摘](reviews/r18-2026-09-02-findings.md) |
...
- 全方向モデルの単体試験10件と閉ループ15件。閉ループのうち5件は同一世界を差動二輪参照設定でも走らせて
