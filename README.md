...
3. 並進加速度窓内の `v` と `[-w_max, w_max]` の `w` を組み合わせる。停止、回頭後直進、
   前進不能時の短い後退も候補に含める。
4. 候補円弧上の最初の車体衝突点までに停止できない候補を棄却する。
5. 視線内のローカルゴール、左右クリアランス、狭所の左右差、方位、前回操舵との差、側方圧迫で
   候補を採点する。
6. 緊急境界に近いほど速度上限を落とし、最良候補と診断量を返す。
...
BAC は全40 episodeを完走した一方、成功時平均到達時間は41.9秒で、DWB 25.3秒、MPPI 28.6秒、
RPP 24.7秒より遅い結果でした（controller ごとに成功集合が違うため時間は参考値）。BAC の利点が
出たのは、強い経路横ずれを与えた狭路と、経路上へ後から障害物を出した条件です。数字を一般的な
成功確率とは解釈しないでください。
