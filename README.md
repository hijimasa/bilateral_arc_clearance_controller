...
- 自己位置ズレ 0.10〜0.25 m の 1.5 m 通路で**到達時間もクリアランスも不変**(28.8 s / 0.22〜0.23)。
  同じ 0.25 m で DWB は衝突、RPP は中断し、プランナ側のマージン対策でも「依然衝突」か
  「計画不能」にしかならない

単一ポーズ推定のグローバル経路を有限周期で更新する標準的な構成では、この頑健性をプランナ側
だけで代替できません。プランナは誤ったポーズ推定の上でしか計画できず(ロボット座標系の観測を
制御周期で使う実行層は地図↔オドメトリ間の座標系誤差の影響を受けない)、再計画の合間の窓を
埋められず、近接減速はそもそもプランナの語彙にないためです。検証は
...
28.7 秒でした（DWB 24.7 秒 (50/54 成功・衝突 2)、MPPI 28.4 秒 (51/54)、RPP 24.2 秒 (48/54)。
