...
同一条件ベンチマーク(18 シナリオ・90 episode)での実測:

- 障害物 0.10 m 以内への接近ゼロ、衝突ゼロ(標準 3 controller は成功 episode でも数 cm の掠りが発生)
- 速度は障害物への近接に単調応答: 余裕があれば巡航、狭ければ減速、安全マージン床を下回る幅には進入しない
- 行き詰まりでは凍結せず、安全に後退して待つ
- 自己位置が 0.15 m ずれた 1.5 m 通路を、無対策の標準 controller は衝突・中断し、プランナ側の
  マージン対策でも「依然衝突」か「計画不能」にしかならない中で、唯一完走する

この頑健性はグローバルプランナでは原理的に代替できません。プランナは誤ったポーズ推定の上で
しか計画できず(実行層はセンサ直結で座標系誤差に免疫)、再計画の合間の窓を埋められず、近接
減速はそもそもプランナの語彙にないためです。検証は
...
要点は、BAC が通常条件で最速の controller ではないことです。手元の同一条件ベンチマーク
（18 シナリオ）では BAC は全90 episodeを完走し、成功時平均到達時間は29.4秒でした
（DWB 24.6秒 (40/46成功)、MPPI 28.4秒 (42/46)、RPP 24.1秒 (40/46)。controller ごとに
成功集合が違うため時間は参考値）。BAC の利点が出たのは、強い経路横ずれを与えた狭路と、
経路上へ後から障害物を出した条件で、これらは追従型 controller が全滅または衝突しました。
