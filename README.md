...
BAC は最速の controller を目指していません。目的は、上流の自己位置推定・地図・グローバル
プランナ・再計画レートが理想的でない現実環境で、**止まらず・ぶつからず・説明可能に動き続ける**
ことです。グローバルプランは「意図の方向」としてだけ使い、実際に通る場所は生センサの幾何から
決めるため、経路が通路中心からずれても、再計画前の経路上に障害物が残っても、挙動が破綻しません。
自己位置推定の誤差への耐性はこの設計の帰結の一つです。
...
6. 視線内のローカルゴール、左右クリアランス、狭所の左右差、方位、前回操舵との差、側方圧迫で
   候補を採点する。回頭・停止候補は「回頭後に直進した到達点」で採点し、ゴールから逸れた
   向きで立ち尽くす均衡を排除する。最良候補と診断量を返す。
...
score = 2.0 * min(clearance, adaptive_cap)
      ...
      - 1.0 * local_goal_distance
      - 0.15 * abs(heading_error)
...
ユニサイクル運動学を含む12本の閉ループシナリオです。シナリオハーネスを直接使うと CSV を残せます。
...
0.15 m ずれた経路、経路上障害物、密集障害物です。`--filter NAME` と
`--w-clearance/--w-goal-dist/--w-balance/--w-hysteresis` で回帰確認と重み sweep ができます。
...
Nav2 controller 契約を前提にし、plan を robot frame に変換します。生スキャンは TF で base frame
へ変換し、未受信・古い・TF 失敗時は costmap の lethal セルへフォールバックします。
...
16シナリオ比較結果と限界を [既存手法との比較](docs/method_comparison.md) に整理しました。

要点は、BAC が通常条件で最速の controller ではないことです。手元の同一条件ベンチマークでは
BAC は全48 episodeを完走し、成功時平均到達時間は29.3秒でした（DWB 25.3秒 (34/40成功)、
MPPI 28.6秒 (36/40)、RPP 24.7秒 (34/40)。controller ごとに成功集合が違うため時間は参考値）。
BAC の利点が出たのは、強い経路横ずれを与えた狭路と、経路上へ後から障害物を出した条件で、
これらは追従型 controller が全滅または衝突しました。数字を一般的な成功確率とは解釈しないで
ください。
...
- 角速度候補は全範囲から選ぶ。実機の角加速度・jerk は下位速度制御器で制限する。
