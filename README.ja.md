...
- Nav2が渡すgoal姿勢に従えるのは全方向モデルだけです。ヨーで操舵する差動二輪とAckermannは方位を
  進行方向と独立に選べないため、goal姿勢を無視して経路接線に従います。全方向モデルはさらに
  `plan_yaw_mode: "plan"`で走行中の姿勢そのものをplanのpose orientation列に委ねられます
  （[docs/nav2_integration.md](docs/nav2_integration.md)参照）。
